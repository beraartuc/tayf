...
/// [`crate::style::Color::to_toml_str`]. The `[general]` section uses
...
pub(crate) fn default_config_toml() -> String {
    ...
    let g = crate::config::GeneralSection::default();
    ...
    // `theme` and `profile` default to `None` (no preset). Omit them
    // from the dump so the canonical TOML representation matches the
    // shipped `GeneralSection::default()` (both fields are `#[serde(default)]`).
    let _ = writeln!(out);
    for r in crate::rules::builtin_rules() {
        let _ = writeln!(out, "[[rules]]");
        let _ = writeln!(out, "name = \"{}\"", r.name);
        // Triple-single-quoted literal strings preserve regex backslashes
        // verbatim; mirrors the toml_edit serialization fold used by
        // `reconcile.rs` for new rule writes.
        let _ = writeln!(out, "pattern = '''{}'''", r.pattern);
        // Style serialization: emit only non-default axes so the dump
        // stays minimal and roundtrips cleanly through `UserStyle`.
...
mod tests {
    ...
    fn default_config_toml_emits_every_builtin_name_in_order() {
        // v0.6.1 §3.3: default_config_toml is the source of truth for
        // the Shift+D first-run init body. Every BUILTIN_NAMES entry
        // must appear, in the same order as builtin_rules().
        ...
        // Sanity-check the [general] preamble shape.
    ...
    fn default_config_toml_roundtrips_through_config_parse() {
        // v0.6.1 §3.3: the dumped body must parse back through the
        // shipped config::parse path without errors — otherwise the
        // Shift+D flow would write an unreadable config.
        let body = default_config_toml();
        let cfg = crate::config::parse("(dump)", &body).expect("dump parses");
