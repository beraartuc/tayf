...
//!
//! The v0.5.3 profile-active benches are retired with the embedded profile
//! library (v0.12.0): the six domain rules are now built-ins, so the
//! default-path benches above exercise the always-on promoted rules.
//!
...
// The v0.5.3 profile-active benches (`bench_profile_{aws,k8s,docker,gcp,
// network}`) are retired with the embedded profile library (v0.12.0): the
// six domain rules are now built-ins, so the default-path benches above
// (ipv4-heavy / mixed-syslog / captures-heavy / passthrough) already exercise
// the always-on promoted rules (arn, instance_id) in the substrate.
