...
| 4 | 8 hardcoded pattern: IPv4, IPv6, MAC, log level, HTTP status, FQDN, duration, filename extension (curated catalog) | T4 |
...
| 11 | TUI mode detection alt-screen ile sınırlı değil: bracketed paste (`2004`) ve mouse tracking (`1000`/`1002`/`1003`/`1006`) modları da passthrough tetikler. State machine bitmask tutar. Modern TUI'ler (Claude Code dahil) korunur. | T6 |
...
`Compiled` struct'ı ve 8 hardcoded builtin pattern. Her pattern için match/no-match testi. Filename pattern dinamik olarak inşa edilir (spec §3.8).
...
mod tests {
    ...
    fn filename_longest_match_for_compound_extensions() {
        ...
        // The pattern should prefer "archive.tar.gz" over "archive.tar".
...
const FILENAME_EXTENSIONS: &[&str] = &[
    ...
    // Config & data
    "json", "jsonc", "yaml", "yml", "toml", "ini", "conf", "cfg", "env", "lock",
    "properties", "plist",
    // Source code
    "rs", "py", "pyi", "pyx",
    "js", "mjs", "cjs", "ts", "tsx", "jsx",
    "go", "java", "kt", "kts", "groovy", "scala", "sbt", "clj", "cljs",
    "c", "h", "cpp", "hpp", "cc", "hh", "cxx", "hxx", "m", "mm",
    "cs", "vb", "fs", "fsx",
    "rb", "erb", "php", "phtml", "pl", "pm", "lua",
    "r", "jl", "swift", "dart",
    "ex", "exs", "erl", "hrl", "hs", "lhs", "ml", "mli",
    "elm", "nim", "zig", "v", "sv", "vhdl",
    "sh", "bash", "zsh", "fish", "ksh", "csh", "tcsh", "ps1", "bat", "cmd",
    "awk", "sed", "sql", "psql",
    // Web & markup
    "html", "htm", "xhtml", "xml", "xsl", "xslt",
    "css", "scss", "sass", "less", "styl",
    "vue", "svelte", "astro", "mdx", "svg", "wasm", "wat",
    // Images
    ...
    "deb", "rpm", "pkg", "apk", "ipa", "dmg", "iso", "img",
    "vmdk", "qcow2", "vhd", "vhdx", "appimage", "msi", "snap", "flatpak",
];
...
/// (Compiled shape) and §3.8 (filename catalog).
...
Expected: 11 test pass (8 pattern match + filename rejects + filename longest + compile).

- [ ] **Step 6: Commit**

```bash
git add src/rules.rs src/style.rs src/lib.rs
git commit -m "feat(rules): compile eight built-in patterns

Adds the v0.1 hardcoded ruleset (IPv4, IPv6, MAC, log level,
HTTP status, FQDN, duration, filename extension) and the Compiled
struct that holds them. Each pattern is hand-tuned to be linear-
time per CLAUDE.md §3.

The filename pattern is built dynamically from FILENAME_EXTENSIONS
(spec §3.8): a curated catalog of archives, source code, config,
documents, media, and binary formats. Extensions are sorted
longest-first so compound forms like .tar.gz match before .gz.
...
## Task 6: Pipeline (`pipeline.rs`) — TUI Mode SM + Rule Application

Pipeline = TUI mode state machine (alt-screen + bracketed paste + mouse) + line buffer adaptörü + rules application. Spec §3.5'teki state machine ve §3.6'daki apply_rules burada birleşir.

**Files:**
- Create: `src/pipeline.rs`
- Modify: `src/lib.rs`

- [ ] **Step 1: TUI mode SM testleri yaz**
...
mod tui_mode_tests {
    ...
    fn alt_screen_modern_enters_on_1049h() {
        ...
        for &b in b"\x1b[?1049h" { sm.step(b); }
    ...
    fn alt_screen_exits_on_1049l() {
        let mut sm = TuiModeSm::new();
        for &b in b"\x1b[?1049h\x1b[?1049l" { sm.step(b); }
        assert!(!sm.passthrough());
    }
    ...
    fn bracketed_paste_enters_on_2004h() {
        let mut sm = TuiModeSm::new();
        for &b in b"\x1b[?2004h" { sm.step(b); }
        assert!(sm.passthrough());
    }
    ...
    fn multiple_modes_active_simultaneously() {
        let mut sm = TuiModeSm::new();
        for &b in b"\x1b[?2004h\x1b[?1000h" { sm.step(b); }
        assert!(sm.passthrough());
        // Clearing only one mode keeps passthrough on the other.
        for &b in b"\x1b[?2004l" { sm.step(b); }
        assert!(sm.passthrough(), "mouse mode still on");
        for &b in b"\x1b[?1000l" { sm.step(b); }
    ...
    fn split_across_chunks_still_triggers() {
        let mut sm = TuiModeSm::new();
        for &b in b"\x1b[?104" { sm.step(b); }
        assert!(!sm.passthrough());
        for &b in b"9h" { sm.step(b); }
        assert!(sm.passthrough());
    }
    ...
    fn unknown_dec_private_mode_does_not_trigger() {
        let mut sm = TuiModeSm::new();
        // \x1b[?25h is "show cursor" — common, not a TUI indicator.
        for &b in b"\x1b[?25h" { sm.step(b); }
        assert!(!sm.passthrough());
    }
...
mod pipeline_tests {
    ...
    fn bracketed_paste_bypasses_rules() {
        ...
        let mut pipe = Pipeline::new(compiled);
        let mut out = Vec::new();
        // Enter bracketed paste, then output a filename that would otherwise color.
        pipe.feed(b"\x1b[?2004h", &mut out).unwrap();
        pipe.feed(b"claude.md\n", &mut out).unwrap();
        assert!(out.windows(9).any(|w| w == b"claude.md"));
        let post_toggle = &out[8..]; // skip the \x1b[?2004h
        assert!(!post_toggle.windows(2).any(|w| w == b"\x1b["));
    }
...
pub(crate) struct TuiModeSm {
    ...
    flags: u32, // any non-zero ⇒ passthrough
}
...
impl Pipeline {
    ...
    pub(crate) fn feed<W: Write>(&mut self, chunk: &[u8], out: &mut W) -> std::io::Result<()> {
        ...
        while cursor < chunk.len() {
            ...
            if pass_before {
                // We were in TUI mode for the whole segment — passthrough.
...
git commit -m "feat(pipeline): TUI mode state machine and rule application

Pipeline orchestrates output processing. A 5-state DEC private mode
parser tracks a bitmask of active TUI modes — alt-screen
(47/1047/1049), bracketed paste (2004), and mouse tracking
(1000/1002/1003/1006). Any non-zero flag puts the pipeline in
passthrough so vim, htop, less, gum, neovim, Claude Code, lazygit,
and other modern TUIs render correctly.

In normal mode, LineBuffer accumulates bytes and apply_rules wraps
the first non-overlapping match of each rule in SGR (spec §3.5,
§3.6)."
```
...
- [ ] **Step 2: TUI mode smoke (alt-screen + bracketed paste + mouse)**

```bash
./target/release/tayf
# içeride sırayla:
vim /tmp/test.txt              # alt-screen
# yaz, esc, :wq
less /etc/services             # alt-screen
# q ile çık
htop                           # alt-screen + mouse (genelde)
# q ile çık
claude                         # Claude Code TUI — alt-screen + bracketed paste
# /exit veya Ctrl+C ile çık
# Yapıştırma testi: shell'de Ctrl-V ile uzun bir metin yapıştır (bracketed paste)
exit
```

Beklenen:
- vim/less/htop/Claude Code görsel olarak bozulmaz; renk leak'i yok.
- Çıkıştan sonra prompt temiz, terminal kullanılabilir.
- Yapıştırılan metin olduğu gibi shell'e girer (bracketed paste içinde regex enjekte edilmez).
...
| 8 hardcoded pattern (incl. filename catalog) | T4 |
| Shell discovery cascade | T7 |
| CLI flag'leri | T11 |
| isatty bypass | T8, T16 (Tayf::run apply_colors) |
| TUI mode detection (alt-screen + bracketed paste + mouse) | T6 |
| TAYF_LOG tracing | T9 |
